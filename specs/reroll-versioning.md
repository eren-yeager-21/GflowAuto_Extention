...
8. Starting a re-roll snapshots the frame's current image URL, Flow tile title, and completion time.
9. The snapshot becomes the revert version only after the new generated image is captured successfully.
10. A failed re-roll leaves the current image and existing revert version intact.
11. After a successful re-roll, the frame displays a Revert button.
12. Revert restores the previous image and Flow tile title, clears the one-level revert history, and overwrites the target file with the restored image.
13. Revert changes the extension mapping and downloaded file; it does not delete either generated tile from Google Flow.
