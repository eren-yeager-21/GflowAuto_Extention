...
(function () {
  ...
  function getReferenceGuidanceStatement(charRefs = [], frameRefName = null, referenceStatement = null, continuity = null) {
    if (referenceStatement && String(referenceStatement).trim()) {
      return String(referenceStatement).trim();
    }
    ...
    if (hasChars && hasFrame) {
      ...
      return `## Use provided reference strictly for visual style, stick figure anatomy, line weight, and character proportions.\n## Use the provided frame reference${framePart} to maintain continuous scene layout, camera perspective, lighting, color palette, and environmental coherence for generating the current frame.`;
    } else if (hasChars && !hasFrame) {
      return `## Use provided reference strictly for visual style, stick figure anatomy, line weight, and character proportions. Render all scene environment, actions, and props as specified in this prompt.`;
    } else if (!hasChars && hasFrame) {
      const framePart = cleanFrame ? ` ${cleanFrame}` : '';
      return `## Use the provided frame reference${framePart} to maintain continuous scene layout, camera perspective, lighting, color palette, and environmental coherence for generating the current frame.`;
    }
    return '';
  }
