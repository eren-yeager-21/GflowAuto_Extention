...
Set max_parallel_generations at the top level of the JSON to control active independent image generations. The default is 3, and the side panel dropdown accepts values from 1 through 10. The extension uses one prompt-box submitter and keeps up to that many accepted independent generations active. Frames with frame_reference or continuity set to continue remain in source order and run one at a time after their referenced frame completes. Pause finishes and maps already accepted generations before stopping; untouched frames remain Pending. The Spec Pipeline also includes Settings and a copyable Debug Log, with Text to Image and Nano Banana 2 as the fixed defaults.
